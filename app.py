...
def load_html(search_date=None):
    ...
    try:
        ...
        print(f"Otro error al cargar HTML: {e}")
        return []
...
def scraping(search_date=None, search_lotery=None):
    ...
    try:
        ...
        print("Error: El archivo 'lottery.json' no fue encontrado.")
        return []
    except json.JSONDecodeError:
        print("Error: El archivo 'lottery.json' contiene JSON inválido.")
        return []

    print(f"Tipo de 'data' después de cargar JSON: {type(data)}")
    if isinstance(data, list) and len(data) > 0:
        print(f"Primer elemento de 'data': {data[0]}")
    elif isinstance(data, dict) and 'lotteries' in data and isinstance(data['lotteries'], list) and len(data['lotteries']) > 0:
        data = data['lotteries']  # Acceder a la lista de loterías
        print(f"Primer elemento de 'data['lotteries']': {data[0]}")
    else:
        print(f"'data' no tiene la estructura esperada: {data}")
        return []  # Salir si la estructura del JSON es incorrecta
    ...
    for game_block in games_blocks:
        ...
        if title:
            ...
            continue  # Saltar si no se encuentra el título
        ...
        pather_date = game_block.find("div", "session-date")
        if pather_date:
            pather_date = pather_date.getText().strip()
        else:
            continue  # Saltar si no se encuentra la fecha
...
def scrapingByName(search_name, search_date=None, search_lotery=None):
    ...
    if isinstance(data, dict) and 'lotteries' in data and isinstance(data['lotteries'], list):
        data = data['lotteries']
    elif isinstance(data, list):
        pass  # Ya es una lista
    else:
        print("Error: Estructura JSON inesperada en 'lottery.json'.")
        return []
...
def search_lotery19():
    search_date = request.args.get('date', datetime.datetime.now().strftime("%d-%m-%Y"))
    data = scrapingByName("americanas/new-york-medio-dia", search_date, "New York Medio Día")  # Ajustar nombre
    return JsonUFT8(data)
